...
router.get('/', async (req, res) => {
  try {
    ...
    res.render('home.html', { categorias, carrito, usuario });
...
router.get('/categoria/:categoria', async (req, res) => {
  try {
    ...
    res.render('categoria.html', { categoria, productos, carrito, usuario });
  } catch (err) {
    res.status(500).send({ err });
  }
});
...
router.post('/buscar', async (req, res) => {
  try {
    ...
    res.render('buscar.html', { productos, query, carrito, usuario });
  } catch (err) {
    res.status(500).send({ err });
  }
});
...
router.get("/carrito", (req, res) => {
  ...
  console.log('Username en carrito:', req.username);
